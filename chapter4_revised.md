...
**触发阈值策略**（DI 阈值 0.20 / 0.35 / 0.50 基于回放数据的分位数标定：P75=0.20, P90=0.35, P95=0.50，详见本章[数据摘要](#数据摘要)）：
...
class AgentClaim:
    ...
    peer_penalty: float = 1.0      # Peer Review降权折扣因子（默认1.0=无惩罚）
...
class DivergenceDetector:
    ...
    def analyze(self, claims: List[AgentClaim]) -> DivergenceReport:
        ...
        # 策略选择（阈值基于回放数据分位数标定 P75/P90/P95）
        if di < 0.20:
...
class VoteAggregator:
    ...
    def compute_weights(self, claims: List[AgentClaim]) -> np.ndarray:
        """
        权重公式: w_i = exp(-α · MAPE_i) · peer_penalty_i / Σ_j (...)
        
        小样本惩罚: 当sample_count < 30时，额外施加不确定性折扣
        Peer Review惩罚: peer_penalty ∈ [0.3, 1.0]，通过Peer Review打分确定
        """
        ...
        # Peer Review显式惩罚（peer_penalty默认为1.0，仅Peer Review场景下调低）
        peer_penalties = np.array([c.peer_penalty for c in claims])
        adjusted = raw_weights * sample_discounts * peer_penalties
        return adjusted / adjusted.sum()
    
    def simple_mean(self, claims: List[AgentClaim]) -> Tuple[float, Tuple[float, float]]:
        """
        简单算术均值融合（用于DI < 0.20低分歧场景）
        """
        forecasts = np.array([c.point_forecast for c in claims])
        y_hat = float(np.mean(forecasts))
        
        # Bootstrap置信区间（等权重）
        n_bootstrap = 1000
        rng = np.random.RandomState(42)
        bootstrap_means = [
            float(np.mean(rng.choice(forecasts, size=len(forecasts), replace=True)))
    ...
    def peer_review_adjust(
        ...
        3. 低分Agent设置显式 peer_penalty 折扣因子（最低0.3）
        
        与旧版区别：不再通过修改 historical_mape 间接降权，
        而是使用显式 peer_penalty 字段，语义更清晰。
        """
        n = len(claims)
        ...
        for i, claim in enumerate(claims):
            if peer_scores[i] < mean_score - std_score and std_score > 0:
                penalty = max(0.3, peer_scores[i] / mean_score)
                adjusted.append(AgentClaim(
                    ...
                    confidence=claim.confidence * penalty,
                    historical_mape=claim.historical_mape,
                    evidence_refs=claim.evidence_refs,
                    sample_count=claim.sample_count,
                    peer_penalty=penalty,  # 显式设置peer_penalty
                ))
            else:
...
def debate_orchestrator(claims: List[AgentClaim], trace_id: str) -> dict:
    ...
    if report.strategy == Strategy.DIRECT_FUSION:
        # DI < 0.20: 分歧极小，使用简单算术均值（无需加权）
        y_hat, ci = aggregator.simple_mean(claims)
        return {
            "trace_id": trace_id,
...
**关键设计决策**：
- DI 阈值 0.20 / 0.35 / 0.50 基于回放数据的分位数标定（P75=0.20, P90=0.35, P95=0.50），可通过回放数据持续调优
- DI < 0.20 时使用简单算术均值融合（而非加权投票），因为低分歧场景下Agent预测高度一致，加权操作无实质收益且增加不必要的计算
- Peer Review 机制不是开放对话，而是基于区间重叠率和预测偏差的结构化评分，避免第1章指出的"辩论从众"陷阱；降权通过显式 `peer_penalty` 字段施加，语义清晰可审计
...
class PlattCalibrator:
    ...
    def fit(self, window_days: int = 30) -> Tuple[float, float]:
        ...
        """
        # 截取最近 window_days * 10 条记录（假设每天最多10条）
        recent_n = min(len(self._history), window_days * 10)
        recent = self._history[-recent_n:]
        
        if len(recent) < 10:
            return self.A, self.B  # 样本不足，保持原参数
        
        errors = np.array([self._compute_normalized_error(r) for r in recent])
        confidences = np.array([r.raw_confidence for r in recent])
        ...
        self.fitted = True
        self.last_fit_date = datetime.now().strftime("%Y-%m-%d")
    ...
    def calibrate(self, raw_confidence: float, predicted: float, 
        ...
        if expected_error is None:
            # 使用历史平均归一化误差（最近30条）
            if self._history:
                recent = self._history[-min(len(self._history), 30):]
                expected_error = np.mean([
                    self._compute_normalized_error(r) for r in recent
                ])
            else:
    ...
    def should_recalibrate(self, recent_errors: List[float], 
        ...
        # 条件1：距上次校准超过30天
        if self.last_fit_date:
            try:
                last_date = datetime.strptime(self.last_fit_date, "%Y-%m-%d")
...
def run_forecast_example():
    ...
    采用 LangGraph 推荐的 graph.invoke() + GraphInterrupt 异常捕获模式：
    1. 首次 invoke() 执行到 interrupt() 暂停点
    2. 捕获 GraphInterrupt 异常，获取中断信息
    3. 通过 Command(resume=...) 恢复执行
    """
    ...
    # 首次执行：图会自动运行到 interrupt() 暂停点
    try:
        final_state = graph.invoke(initial_state, config=config)
    except GraphInterrupt as e:
        # HITL 暂停点触发 —— LangGraph 保留 checkpoint 状态
        print(f"\n⚠️ HITL Required — Graph interrupted at hitl_escalator node")
        ...
        # 从暂停点恢复执行
        final_state = graph.invoke(human_response, config=config)
...
> **重要声明**：以下报文为基于 [A2A协议](https://a2a-protocol.org/latest/specification/) Task/Message/Artifact 三元模型的物流预测场景适配设计，**非A2A标准JSON-RPC wire format**。实际实现中需按 [A2A JSON-RPC 2.0绑定格式](https://a2aprotocol.ai/docs/guide/a2a-sample-methods-and-json-responses) 封装（如 `jsonrpc: "2.0"`, `method: "tasks/send"` 等标准字段）。本章报文侧重展示物流预测场景的语义层协议设计，具体 wire format 请参考 [A2A Specification](https://a2a-protocol.org/latest/specification/)。

### 4.4.1 Agent间任务委派报文（A2A Task）

L2 Central Nexus → L1 Regional Supervisor 的任务委派（基于 [A2A Task 模型](https://a2a-protocol.org/latest/specification/#task)）：
...
L1 Regional Supervisor → L2 Central Nexus 的预测结果回传（基于 [A2A Artifact 模型](https://a2a-protocol.org/latest/specification/#artifact)）：
...
多Agent分歧时的结构化互评报文（基于 [A2A Message 模型](https://a2a-protocol.org/latest/specification/#message)）：
...
推入 Matrix 群聊的 HITL 请求格式化消息（基于 [Matrix Protocol](https://matrixdocs.github.io/)）：
...
- **发现2：分歧检测应采用DI值分梯度处理而非二值判断**，文献和工程实践均表明"一刀切"的辩论触发会导致过度辩论（浪费Token）或关键场景漏判。DI三区间策略（<0.20/0.20-0.35/0.35-0.50/≥0.50）在物流预测回放中表现最优，DI < 0.20时简单均值融合即足矣、无需加权
- **发现3：Platt Scaling扩展至回归预测的置信度校准是可行的**，基于30天滑动窗口历史误差拟合Logistic回归参数A/B，能将Agent自评置信度映射为更可靠的经验置信度。关键是要配合分桶校准（按场地类型、事件类型、预测维度分桶），因为不同场景的误差分布差异显著
- **发现4：A2A协议的Task/Artifact/Message三元模型可直接映射物流预测场景**，Task对应预测任务，Artifact对应ForecastClaim（预测主张），Message对应Agent间通信。A2A的流式SSE机制还可用于实时推送预测进度；实际部署需按A2A JSON-RPC 2.0标准封装wire format
...
13. [MCP Specification](https://modelcontextprotocol.io/) — Model Context Protocol 官方规范，Agent↔Tool标准化接口
